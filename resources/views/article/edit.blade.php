...
<form style="margin: 2% 2%;">
        ...
        <select id="class" class="form-control">
            ...
            @if($class)
                @foreach($class as $val)
                    @if($data['class'] == $val->wb_c_id)
                        <option class="cid" selected value="{{$val->wb_c_id}}">{{$val->wb_c_name}}</option>
                    @else
    ...
    <textarea class="form-control" id="describe" rows="3">{{$data['describe']}}</textarea>
    ...
    <button id="save" class="btn btn-primary btn-block" type="button">保 存</button>
</form>
@else
    服务器错误，请重试！
@endif
